...
const tokenizerOri = new natural.WordTokenizer();
...
class ContentBasedRecommender {
    ...
    trainOpt(documents, item_id) {
        ...
        console.log ("step 1 ")
        console.time();
        const preprocessDocs = this._preprocessDocumentsOpt(documents, this.options);
        console.timeEnd();
    ...
    trainOpt2(documents) {
        ...
        console.log ("step 2 ")
        console.time();
        this.docVectorsOpt = this._produceWordVectors(preprocessDocs, this.options);
        console.timeEnd();
        return this.docVectorsOpt;
    ...
    validateDocuments(documents) {
        ...
        for (let i = 0; i < documents.length; i += 1) {
            ...
            if (!_.has(document, "id") || !_.has(document, "token")) {
                throw new Error("Documents should be have fields id and token");
            }
        }
    }
    ...
    _preprocessDocumentsOpt(documents, options) {
        if (options.debug) {
    ...
    _getTokensFromString(string) {
        ...
        // tokenize the string
        const tokens = tokenizerOri.tokenize(tmpString);
    ...
    getTokensFromString(string) {
        ...
        tmpString = tmpString.trim().replace(regex, '').replace(/[-&\/\\#,+()$~%.'":*?!<>{}]/g, '');
        // tokenize the string
